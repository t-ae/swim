...
class CorrelationTests: XCTestCase {
    ...
    func testSSD() {
        ...
        XCTAssertEqual(calculateSSD(image1, image1), 0)
        XCTAssertEqual(calculateSSD(image1, image2), 31)
    }
    ...
    func testSAD() {
        ...
        XCTAssertEqual(calculateSAD(image1, image1), 0)
        XCTAssertEqual(calculateSAD(image1, image2), 11)
    }
    ...
    func testNCC() {
        ...
        XCTAssertEqual(calculateNCC(image1, image1), 1)
        XCTAssertEqual(calculateNCC(image1, image2), ip / sqrt(l1 * l2))
    }
    
    func testZNCC() {
        let image1 = Image<Intensity, Float>(width: 3, height: 2, data: [0, 1, 2, 3, 4, 5])
        
        XCTAssertEqual(calculateZNCC(image1, image1), 1)
        
        let i1 = Image<Intensity, Float>(width: 3, height: 1, data: [0, 1, 0])
        let i2 = Image<Intensity, Float>(width: 3, height: 1, data: [1, 0, 1])
        
        XCTAssertEqual(calculateZNCC(i1, i2), -1)
    }
}
